...
let
  ...
  shells = map (sh: path { path = sh; }) (
    map (shell: pkgs.callPackage (core + /lib/nix/shell/${shell}/default.nix) { }) [
      "python"
      "rust"
      "julia"
    ]
...
in
{
      ...
      boot = {
        contents =
          ...
          in
          {
            ...
            "/EFI/Linux/${loader.ukiFile}".source = "${system.build.uki}/${loader.ukiFile}";
  ...
  users = {
    mutableUsers = false;
    users.multisn8.openssh.authorizedKeys.keyFiles = [ ~/.ssh/id_to_elusive.pub ];
  };
