...
let
  ...
  core = ../..;
  shells = map (sh: path { path = sh; }) (map
    (shell: pkgs.callPackage (core + /lib/nix/shell/${shell}/default.nix) { }) [
      "python"
      "rust"
      "julia"
    ]);
  ...
  rootTemplate = pkgs.runCommand "root-template" (rec {
...
in {
  imports =
    [ ../generalized.nix ../development.nix "${modulesPath}/image/repart.nix" ];
  ...
  image.repart = {
    ...
    partitions = {
      boot = {
        contents = let loader = system.boot.loader;
        in {
  ...
  fileSystems = mapAttrs' (name: target:
    ...
    (let cfg = config.image.repart.partitions.${name}.repartConfig;
    in {
